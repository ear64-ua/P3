...
/*
 * Este fichero es similar a PlayerFileTest.java (del corrector de la práctica 5)
 * 
 */
...
public class TestExamen2 {
	...
	/*
	 * Dos cazas, TIEInterceptor (mejorado) + YWing
	 *   TIEInterceptor en (5,5), YWing en (4,4)  (debes cambiar 4,4 por otros valores)
	 *   
	 *   
	 * Ten en cuenta que los demás tests de este fichero no comprueban todo, algunas comprobaciones
	 * se dejaron para hacerlas en los tests de la clase Game. Tus tests deberían hacer todas las
	 * comprobaciones  
	 */
	@Test
	public void testExamen() {
		String si = "1/TIEInterceptor\nimprove 1 99\nlaunch 5 5\ncross 1\nexit";  // quizá debes añadir más cosas a esta cadena
		PlayerFile pfImperial = makeNewPlayerFile(si,Side.IMPERIAL);
		pfImperial.setBoard(gb);
		pfImperial.initFighters();  // TIEInterceptor id=1
		pfImperial.nextPlay(); // improve 1 99 
		pfImperial.nextPlay(); // launch 5 5  (id=1)
		Fighter fi = pfImperial.getGameShip().getFleetTest().get(0); // TIEInterceptor
		
		String sr = "1/YWing\nlaunch 5 1\nexit";  // quizá debes añadir más cosas a esta cadena
		PlayerFile pfRebel = makeNewPlayerFile(sr,Side.REBEL);
		pfRebel.setBoard(gb);
		pfRebel.initFighters();
		pfRebel.nextPlay(); // launch 4 4  (id=2)
		Fighter fr = pfRebel.getGameShip().getFleetTest().get(0); // YWing (4,4)
		
		//System.out.println(gb.toString());  // tablero antes del movimiento
		
		pfImperial.nextPlay();
		
		//System.out.println(gb.toString());  // tablero después del movimiento
		gs = pfImperial.getGameShip();
		
		//fail("completa el test");
	}
	
	//Se posiciona un caza en la 5 5 y otro en la 5 1
	//El de la 5 5 hace cross, se lo encuentra y lo destruye
	//Se comprueba que quede posicionado en la 5 1 
	// y que el otro caza se ha destruido
	@Test
	public void testNorte() {
		...
		assertEquals(new Coordinate(5, 1), fi.getPosition());
		assertTrue(fr.isDestroyed());
		//fail("completa el test");
	}
	...
	public void testSur() {
		...
		assertEquals(new Coordinate(5, 8), fi.getPosition());
		assertTrue(fr.isDestroyed());
		//fail("completa el test");
	}
	...
	public void testOeste() {
		...
		assertEquals(new Coordinate(3, 5), fi.getPosition());
		assertTrue(fr.isDestroyed());
		//fail("completa el test");
	}
	...
	public void testEste() {
		...
		assertEquals(new Coordinate(7, 5), fi.getPosition());
		assertTrue(fr.isDestroyed());
		//fail("completa el test");
	}
	
	//Prueba unitaria 2, el que hace cross pierde
	//se comprueba que fi es destruido y fr sigue en la misma posicion y no esta destruido
	@Test
	public void testCross2() {
		String si = "1/TIEBomber\nlaunch 5 5\ncross 1\nexit";  // quizá debes añadir más cosas a esta cadena
		PlayerFile pfImperial = makeNewPlayerFile(si,Side.IMPERIAL);
		pfImperial.setBoard(gb);
		pfImperial.initFighters();  // TIEInterceptor id=1
		pfImperial.nextPlay(); // launch 5 5  (id=1)
		...
		assertEquals(new Coordinate(3, 5), fr.getPosition());
		assertTrue(fi.isDestroyed());
		assertFalse(fr.isDestroyed());
		//fail("completa el test");
	}
	
	//Prueba unitaria 3
	//Se encuentra con un amigo y cambia de recorrido (no mata a un enemigoque habia detras)
	//Sigue recorriendo en cruz,elimina a un enemigo al oeste y termina
	//Se comprueba que el fr2 ha sido destruido, el fi ocupa su posicion y fr está intacto
	@Test
	public void testCross3() {
		String si = "2/TIEInterceptor\nimprove 1 99\nlaunch 5 5\nlaunch 5 3\ncross 1\nexit";  // quizá debes añadir más cosas a esta cadena
		PlayerFile pfImperial = makeNewPlayerFile(si,Side.IMPERIAL);
		pfImperial.setBoard(gb);
		pfImperial.initFighters();  // TIEInterceptor id=1
		pfImperial.nextPlay(); // improve 1 99 
		...
		pfRebel.nextPlay(); // launch 4 4  (id=2)
		Fighter fr = pfRebel.getGameShip().getFleetTest().get(0); // YWing (4,4)
		Fighter fr2 = pfRebel.getGameShip().getFleetTest().get(1); // YWing (4,4)
		...
		assertEquals(new Coordinate(3, 5), fi.getPosition());
		assertFalse(fr.isDestroyed());
		assertTrue(fr2.isDestroyed());
	}
	
	private PlayerFile makeNewPlayerFile(String inputFighter,Side side) { 
		StringReader stringReader = new StringReader(inputFighter);
